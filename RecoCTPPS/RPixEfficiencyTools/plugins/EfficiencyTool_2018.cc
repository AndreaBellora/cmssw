...
class EfficiencyTool_2018 : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
	...
	std::map<CTPPSPixelDetId,TH1D*> h1X0ResolutionAllCls2_;
	std::map<CTPPSPixelDetId,TH1D*> h1Y0ResolutionAllCls2_;
...
EfficiencyTool_2018::~EfficiencyTool_2018(){
	for(const auto & rpId :  romanPotIdVector_){
		...
		delete h1X0ResolutionAllCls2_[rpId];
		delete h1Y0ResolutionAllCls2_[rpId];
...
void EfficiencyTool_2018::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	...
	for(const auto & rpPixeltrack : *pixelLocalTracks){
		...
		if(h2TrackHitDistribution_.find(rpId)==h2TrackHitDistribution_.end()){
			...
			h1X0ResolutionAllCls2_[rpId] = new TH1D(Form("h1X0ResolutionAllCls2_arm%i_st%i_rp%i",arm,station,rp),
			Form("h1X0ResolutionAllCls2_arm%i_st%i_rp%i; #sigma_{x} (mm);",arm,station,rp),100,0,0.1);
			h1Y0ResolutionAllCls2_[rpId] = new TH1D(Form("h1Y0ResolutionAllCls2_arm%i_st%i_rp%i",arm,station,rp),
			Form("h1Y0ResolutionAllCls2_arm%i_st%i_rp%i; #sigma_{y} (mm);",arm,station,rp),100,0,0.1);
		...
		for(const auto & pixeltrack : rpPixeltrack){
			...
			for(const auto & planeHits : fittedHits){
				...
				for(const auto & hit : planeHits){
					...
					if(hit.getIsRealHit()){
							double hitX0 = hit.getGlobalCoordinates().x();
							double hitY0 = hit.getGlobalCoordinates().y();
							double hitX0_rotated = 0;
							double hitY0_rotated = 0;
						if(station == 0){
							hitX0_rotated = hit.getGlobalCoordinates().x()*TMath::Cos((-8./180.)*TMath::Pi()) - hit.getGlobalCoordinates().y()*TMath::Sin((-8./180.)*TMath::Pi());
							hitY0_rotated = hit.getGlobalCoordinates().x()*TMath::Sin((-8./180.)*TMath::Pi()) + hit.getGlobalCoordinates().y()*TMath::Cos((-8./180.)*TMath::Pi());
						}	
			...
			if(numberOfFittedPoints == 3) h23PointsTrackHitDistribution_[rpId]->Fill(pixelX0,pixelY0);
			if (arm != 0 && station != 2){
				if (numberOfRowCls2 == 6) h1Y0ResolutionAllCls2_[rpId]->Fill(pixeltrack.getY0Sigma());
				if (numberOfColCls2 == 6) h1X0ResolutionAllCls2_[rpId]->Fill(pixeltrack.getX0Sigma());
			}
			else{
				if (numberOfRowCls2 == 5) h1Y0ResolutionAllCls2_[rpId]->Fill(pixeltrack.getY0Sigma());
				if (numberOfColCls2 == 5) h1X0ResolutionAllCls2_[rpId]->Fill(pixeltrack.getX0Sigma());
			}
			...
			for(const auto pln : listOfPlanes_){
				...
				if(station == 0){
					hitX0_rotated = hitOnPlane[0].getGlobalCoordinates().x()*TMath::Cos((-8./180.)*TMath::Pi()) - hitOnPlane[0].getGlobalCoordinates().y()*TMath::Sin((-8./180.)*TMath::Pi());
					hitY0_rotated = hitOnPlane[0].getGlobalCoordinates().x()*TMath::Sin((-8./180.)*TMath::Pi()) + hitOnPlane[0].getGlobalCoordinates().y()*TMath::Cos((-8./180.)*TMath::Pi());
				}	
...
void EfficiencyTool_2018::endJob(){
	...
	for(const auto & rpId :  romanPotIdVector_){
		...
		if(supplementaryPlots){
			h1X0ResolutionAllCls2_[rpId]->Write();
			h1Y0ResolutionAllCls2_[rpId]->Write();
		...
		if(supplementaryPlots && station != 0){
			...
			h2TrackHitDistribution_[rpId]->SaveAs("pippo.root");
