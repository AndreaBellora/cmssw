...
TtDecayChannelSelector::TtDecayChannelSelector(const edm::ParameterSet &cfg)
    : invert_(cfg.getParameter<bool>("invert")), allowElectron_(false),
      allowMuon_(false), allow1Prong_(false), allow3Prong_(false) {
  ...
  if (restrictTauDecays_) {
    edm::ParameterSet allowedTauDecays =
        cfg.getParameter<edm::ParameterSet>("restrictTauDecays");
    ...
    allowElectron_ = (allowedTauDecays.existsAs<bool>("electron")
                          ? allowedTauDecays.getParameter<bool>("electron")
                          : false);
    allowMuon_ = (allowedTauDecays.existsAs<bool>("muon")
                      ? allowedTauDecays.getParameter<bool>("muon")
                      : false);
    allow1Prong_ = (allowedTauDecays.existsAs<bool>("oneProng")
                        ? allowedTauDecays.getParameter<bool>("oneProng")
                        : false);
    allow3Prong_ = (allowedTauDecays.existsAs<bool>("threeProng")
                        ? allowedTauDecays.getParameter<bool>("threeProng")
                        : false);
  }
  // allowed top decays PSet
  edm::ParameterSet allowedTopDecays =
      cfg.getParameter<edm::ParameterSet>("allowedTopDecays");

  // fill decayBranchA_
  edm::ParameterSet decayBranchA =
      allowedTopDecays.getParameter<edm::ParameterSet>("decayBranchA");
  ...
  // fill decay branchB_
  edm::ParameterSet decayBranchB =
      allowedTopDecays.getParameter<edm::ParameterSet>("decayBranchB");
...
bool TtDecayChannelSelector::
operator()(const reco::GenParticleCollection &parts,
           std::string inputType) const {
  bool verbose =
      false; // set this to true for debugging and add TtDecayChannelSelector
            // category to the MessageLogger in your cfg file
  unsigned int iLep = 0;
  unsigned int iTop = 0, iBeauty = 0, iElec = 0, iMuon = 0, iTau = 0;
  for (reco::GenParticleCollection::const_iterator top = parts.begin();
       top != parts.end(); ++top) {
    if (search(top, TopDecayID::tID, inputType)) {
      ++iTop;
      for (reco::GenParticle::const_iterator td = top->begin();
           td != top->end(); ++td) {
        if (search(td, TopDecayID::bID, inputType)) {
          ++iBeauty;
        }
        if (search(td, TopDecayID::WID, inputType)) {
          for (reco::GenParticle::const_iterator wd = td->begin();
               wd != td->end(); ++wd) {
            ...
            if (search(wd, TopDecayID::WID, inputType)) {
              for (reco::GenParticle::const_iterator swd = wd->begin();
                   swd != wd->end(); ++swd) {
                if (std::abs(swd->pdgId()) == TopDecayID::elecID) {
                  ++iElec;
                }
                if (std::abs(swd->pdgId()) == TopDecayID::muonID) {
                  ++iMuon;
                }
                if (std::abs(swd->pdgId()) == TopDecayID::tauID) {
                  if (restrictTauDecays_) {
                    // count as iTau if it is leptonic, one-prong
                    // or three-prong and ignore increasing iLep
                    // though else
  ...
  if ((iTop == 2) && (iBeauty == 2)) {
    if (channel == iLep) {
      ...
      if (channel == 1) {
        ...
        accept = (iElec && allowedDecays_[Elec]) ||
                 (iMuon && allowedDecays_[Muon]) ||
                 (iTau && allowedDecays_[Tau]);
      }
      if (channel == 2) {
        if (checkSum(allowedDecays_) == channel) {
          // no redundancy
          accept = (allowedDecays_[Elec] == (int)iElec) &&
                   (allowedDecays_[Muon] == (int)iMuon) &&
                   (allowedDecays_[Tau] == (int)iTau);
        } else {
          ...
          if (iElec + iMuon + iTau != channel) {
            ...
            if ((iElec == 2) || (iMuon == 2) || (iTau == 2)) {
              ...
              // two different leptons: look if there is a possible
              // combination
              accept = (((iElec && decayBranchA_[Elec]) &&
                         ((iMuon && decayBranchB_[Muon]) ||
                          (iTau && decayBranchB_[Tau]))) ||
                        ((iMuon && decayBranchA_[Muon]) &&
                         ((iElec && decayBranchB_[Elec]) ||
                          (iTau && decayBranchB_[Tau]))) ||
                        ((iTau && decayBranchA_[Tau]) &&
                         ((iElec && decayBranchB_[Elec]) ||
                          (iMuon && decayBranchB_[Muon]))));
            }
          }
...
bool TtDecayChannelSelector::search(
    reco::GenParticleCollection::const_iterator &part, int pdgId,
    std::string &inputType) const {
  if (inputType == kGenParticles) {
    return (std::abs(part->pdgId()) == pdgId &&
            (part->status() == TopDecayID::unfrag || part->status() == 22))
               ? true
               : false;
  } else {
...
bool TtDecayChannelSelector::search(reco::GenParticle::const_iterator &part,
                                    int pdgId, std::string &inputType) const {
  if (inputType == kGenParticles) {
    return (std::abs(part->pdgId()) == pdgId &&
            (part->status() == TopDecayID::unfrag || part->status() == 23 ||
             part->status() == 52 || part->status() == 1 ||
             part->status() == 22))
               ? true
               : false;
  } else {
...
TtDecayChannelSelector::countProngs(const reco::Candidate &part) const {
  ...
  int prong = 0;
  for (reco::Candidate::const_iterator daughter = part.begin();
...
bool TtDecayChannelSelector::tauDecay(const reco::Candidate &tau) const {
  ...
  for (reco::Candidate::const_iterator daughter = tau.begin();
       daughter != tau.end(); ++daughter) {
  ...
  return ((allowElectron_ && electronTau) || (allowMuon_ && muonTau) ||
