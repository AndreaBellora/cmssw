  ...
  for (auto &pixelDetId : romanPotIdVector_) {
    ...
    if (mux[pixelDetId] == 1) {
      ...
      for (auto &proton : *protons) {
        ...
        if (trackLite.getRPId() != pixelDetId.rawId()) {
          continue;
        }
        foundTrack = true;
        numberOfHitsInTrack = trackLite.getNumberOfPointsUsedForFit();
        ...
        for (auto &pixelTrack : rpPixelTracks) {
          if (TMath::Abs(pixelTrack.getX0() - trackLite.getX()) < 0.1 &&
              TMath::Abs(pixelTrack.getY0() - trackLite.getY()) < 0.1) {
            track = pixelTrack;
        ...
        for (auto &fittedHitDs : track.getHits()) {
