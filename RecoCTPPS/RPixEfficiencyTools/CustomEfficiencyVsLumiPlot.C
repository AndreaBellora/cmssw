...
void CustomEfficiencyVsLumiPlot(){
	...
	TFile *outputFile = new TFile("CustomEfficiencyVsLumiPlot.root","CREATE");
	...
	for(auto & run : runs){
		...
		string avgEfficiencyFileName = Form("OutputFiles/FirstBunchAvgEfficiency_Run%i.dat",run);
	...
	mgAvgEfficiencyPeakFitted->SetMinimum(0);
	mgRelativeAvgEfficiencyPeakFitted->SetMinimum(0);
	...
	mgControlAreaEfficiency->SetMinimum(0);
	mgRelativeControlAreaEfficiency->SetMinimum(0);
	...
	TMultiGraph *mgYAlignmentSensorEdge = new TMultiGraph("mgYAlignmentSensorEdge","Y Alignment @ Sensor Edge; Integrated Luminosity (fb^{-1}); Pixels Used");
	...
	for(auto & arm : arms ){
		for(auto & station : stations){
			...
			gAvgEfficiencyPeakFitted[pair<int,int>(arm,station)]->SetNameTitle(Form("gCustomAvgEfficiencyPeakFitted_arm%i_st%i",arm,station), Form("Arm %i Station %i",arm,station));
			...
			gControlAreaEfficiency[pair<int,int>(arm,station)]->SetNameTitle(Form("gControlAreaEfficiency_arm%i_st%i",arm,station), Form("Arm %i Station %i",arm,station));
			gRelativeControlAreaEfficiency[pair<int,int>(arm,station)] = new TGraph();
			gRelativeControlAreaEfficiency[pair<int,int>(arm,station)]->SetNameTitle(Form("gControlAreaEfficiency_arm%i_st%i",arm,station), Form("Arm %i Station %i",arm,station));
			...
			gYAlignmentSensorEdge[pair<int,int>(arm,station)]->SetNameTitle(Form("gYAlignmentSensorEdge%i_st%i",arm,station), Form("Y Alignment Arm %i Station %i",arm,station));

			for(auto & run : runs){
				if(run == 315840 && arm == 0 && station == 0) continue; // Exclude the run where the pot wasn't inserted
				if(run == 324747 && arm == 1 && station == 2) continue; // Exclude the run with SEU
	...
	cAvgEfficiencyPeakFitted->BuildLegend(0.1,0.1,0.3,0.3);
	...
	cRelativeAvgEfficiencyPeakFitted->BuildLegend(0.1,0.1,0.3,0.3);
	...
	cControlAreaEfficiency->BuildLegend(0.1,0.1,0.3,0.3);
	...
	cRelativeControlAreaEfficiency->BuildLegend(0.1,0.1,0.3,0.3);
	...
	cPixelsUsedForAvgEfficiencyPeakFitted->BuildLegend(0.1,0.1,0.3,0.3);
	...
	cYAlignmentSensorEdge->BuildLegend(0.1,0.1,0.3,0.3);
