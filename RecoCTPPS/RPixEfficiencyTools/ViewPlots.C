...
void ViewPlots(Int_t RunNumber){
	...
	map <std::pair<int,int>,vector<int> > areaForAvgEfficiency;
	...
	fitRanges[std::pair<int,int>(1,0)] = {6.1,17.2,6.1,11.1,6.0,8.5};
	fitRanges[std::pair<int,int>(1,2)] = {44.1,55,44.1,52.5,44,47};

	// Sensor edge x position
	map< std::pair<int,int>, double> sensorEdgeX;
	sensorEdgeX[std::pair<int,int>(0,0)] = 6.85;
	sensorEdgeX[std::pair<int,int>(0,2)] = 43.80;
	sensorEdgeX[std::pair<int,int>(1,0)] = 5.98;
	sensorEdgeX[std::pair<int,int>(1,2)] = 44.08;
	...
	// TCanvas *cAvgEfficiencyFitted = new TCanvas("cAvgEfficiencyFitted","AvgEfficiencyFitArea",screen_x,screen_y,screen_w,screen_h);
	...
	avgEfficiencyOutputFile << "# arm station avgEfficiency avgEfficiencyFitted xMaxFitted pixelsUsedForAvgEfficiencyFitted xMaxFix yParam[0] yParam[1] yFix sigmaParam[0] sigmaParam[1]"<<endl;
	...
	for(auto & arm : arms){
		for(auto & station : stations){
			...
			if(station == 0){
				h2TrackEfficiencyMap = (TH2D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackEfficiencyMap_rotated_arm%i_st%i_rp3",arm,station));
				h1AmplitudeHist = (TH1D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackHitDistribution_rotated_arm%i_st%i_rp3_0",arm,station));
				h1MeanHist = (TH1D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackHitDistribution_rotated_arm%i_st%i_rp3_1",arm,station));
				h1SigmaHist = (TH1D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackHitDistribution_rotated_arm%i_st%i_rp3_2",arm,station));
			}
			else{
				h2TrackEfficiencyMap = (TH2D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackEfficiencyMap_arm%i_st%i_rp3",arm,station));
				h1AmplitudeHist = (TH1D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackHitDistribution_arm%i_st%i_rp3_0",arm,station));
				h1MeanHist = (TH1D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackHitDistribution_arm%i_st%i_rp3_1",arm,station));
				h1SigmaHist = (TH1D*)inputFile->GetDirectory(Form("Arm%i_st%i_rp3",arm,station))->Get(Form("h2TrackHitDistribution_arm%i_st%i_rp3_2",arm,station));
			}

			TH2D* h2AvgEfficiencyArea = new TH2D(*h2TrackEfficiencyMap);

			h1AmplitudeHist->Fit("amplitudeFitFunction","WQ","",fitRanges[std::pair<int,int>(arm,station)].at(0),fitRanges[std::pair<int,int>(arm,station)].at(1));
			...
			h1SigmaHist->Fit("sigmaFitFunction","WQ","",fitRanges[std::pair<int,int>(arm,station)].at(4),fitRanges[std::pair<int,int>(arm,station)].at(5));
			...
			for(int xbin = 1; xbin <= h2TrackEfficiencyMap->GetXaxis()->GetNbins();xbin++){
				...
				if(xMin < xbinCenter && xbinCenter < xMax){
					for(int ybin = 1; ybin <= h2TrackEfficiencyMap->GetYaxis()->GetNbins();ybin++){
						...
						double yMin = meanFitFunction->Eval(xbinCenter) - sigmaFitFunction->Eval(xbinCenter)/2.;
						double yMax = meanFitFunction->Eval(xbinCenter) + sigmaFitFunction->Eval(xbinCenter)/2.;
						if(yMin < ybinCenter && ybinCenter < yMax){
							...
							// h2AvgEfficiencyArea->SetBinContent(xbin,ybin,-1);
						}
					}
				}
			}
			...
			avgEfficiencyOutputFile << arm << " " << station << " " << avgEfficiency << " " << avgEfficiencyFitted << " " << xMax << " " << 
			pixelsUsedForAvgEfficiencyFitted << " " << h2TrackEfficiencyMap->GetXaxis()->GetBinCenter(xbin+wbin) << " " << meanFitFunction->GetParameter(0) << 
			" " << meanFitFunction->GetParameter(1) << " " << h2TrackEfficiencyMap->GetYaxis()->GetBinCenter(ybin) << " " << sigmaFitFunction->GetParameter(0) <<
			" " << sigmaFitFunction->GetParameter(1) << "\n";
