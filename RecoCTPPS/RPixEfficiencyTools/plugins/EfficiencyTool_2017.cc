...
// Class:      EfficiencyTool_2017
//
/**\class EfficiencyTool_2017 EfficiencyTool_2017.cc RecoCTPPS/RPixEfficiencyTools/plugins/EfficiencyTool_2017.cc
...
class EfficiencyTool_2017 : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
	public:
	explicit EfficiencyTool_2017(const edm::ParameterSet&);
	~EfficiencyTool_2017();
...
void EfficiencyTool_2017::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	using namespace edm;
	...
	for(const auto & rpPixeltrack : *pixelLocalTracks){
		...
		if(h2TrackHitDistribution_.find(rpId)==h2TrackHitDistribution_.end()){
		    ...
		    if(supplementaryPlots){
				...
				h1ConsecutivePlanes_[rpId] = new TH1D(Form("h1ConsecutivePlanes_arm%i_st%i_rp%i",arm,station,rp),Form("h1ConsecutivePlanes_arm%i_st%i_rp%i; #sigma_{Ty};",arm,station,rp),2,0,2);
				h1ConsecutivePlanes_[rpId]->GetXaxis()->SetBinLabel(1,"Non-consecutive");
		...
		for(const auto & pixeltrack : rpPixeltrack){
			// if ((pixeltrack.getNDF() +4 )/2 < minNumberOfPlanesForTrack_ || (pixeltrack.getNDF() +4 )/2 > maxNumberOfPlanesForTrack_ ) continue;
			if (Cut(pixeltrack,arm,station)) continue;
			if (!pixeltrack.isValid()) continue;
			...
			if(supplementaryPlots){
				...
				// Sort the vector of planes and require them to be consecutive and fill hist
...
void EfficiencyTool_2017::beginJob(){
...
void EfficiencyTool_2017::endJob(){
...
void EfficiencyTool_2017::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void EfficiencyTool_2017::getPlaneCombinations(const std::vector<uint32_t> &inputPlaneList, uint32_t numberToExtract, 
	std::vector<std::pair<std::vector<uint32_t>,std::vector<uint32_t> > > &planesExtractedAndNot){
...
float EfficiencyTool_2017::probabilityNplanesBlind(const std::vector<uint32_t> &inputPlaneList, int numberToExtract, const std::map<unsigned, float> &planeEfficiency){
	std::vector<std::pair<std::vector<uint32_t>,std::vector<uint32_t> > > planesExtractedAndNot;
...
float EfficiencyTool_2017::efficiencyPartialDerivativewrtPlane(uint32_t plane, const std::vector<uint32_t> &inputPlaneList, int numberToExtract, const std::map<unsigned, float> &planeEfficiency){
...
float EfficiencyTool_2017::errorCalculation(const std::map<uint32_t, float> &planeEfficiency,const std::map<uint32_t, float> &planeEfficiencyError){
...
float EfficiencyTool_2017::probabilityCalculation(const std::map<uint32_t, float> &planeEfficiency){
...
bool EfficiencyTool_2017::Cut(CTPPSPixelLocalTrack track, int arm, int station) {
  uint32_t ndf = 2 * track.getNumberOfPointsUsedForFit() - 4;
  double x = track.getX0();
  double y = track.getY0();

  double maxTx = 0.02;
  double maxTy = 0.02;
  double maxChi2 = TMath::ChisquareQuantile(0.95, ndf);

  if (TMath::Abs(track.getTx()) > maxTx || TMath::Abs(track.getTy()) > maxTy ||
...
DEFINE_FWK_MODULE(EfficiencyTool_2017);