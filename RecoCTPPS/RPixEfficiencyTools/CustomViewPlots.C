...
void CustomViewPlots(Int_t RunNumber, bool noVideoOutput = false){
	bool verbose = false;
	bool additionalPlots = true;
	double dist_fraction = 0.3;
	...
	if(TS1_TS2){ 
		...
		shift = {{0,0},{2,0}};
		yAlignmentRun = 319337;
	}
	if(Post_TS2){ 
		// shift = {{0,-10*TMath::Cos(8./180.)},{2,10}};
		shift = {{0,0},{2,0}};
		yAlignmentRun = 323487;
	}
	...
	vector<float> areaForControlEfficiency_arm0_st0{15.,0.,8.};
	vector<float> areaForControlEfficiency_arm0_st2{52.,0.,8.};
	vector<float> areaForControlEfficiency_arm1_st0{14.,0.,8.};
	vector<float> areaForControlEfficiency_arm1_st2{52.,0.,8.};

	// areaForAvgEfficiency_arm0_st0[1] += shift[0];
	// areaForAvgEfficiency_arm1_st0[1] += shift[0];
	// areaForAvgEfficiency_arm0_st2[1] += shift[0];
	// areaForAvgEfficiency_arm1_st2[1] += shift[0];
	...
	fitRanges[std::pair<int,int>(0,2)] = {44.0,55.1,44.0,47.2,44,47};
	...
	sensorEdgeX[std::pair<int,int>(0,0)] = 6.88;
	sensorEdgeX[std::pair<int,int>(0,2)] = 43.92;
	sensorEdgeX[std::pair<int,int>(1,0)] = 6.02;
	sensorEdgeX[std::pair<int,int>(1,2)] = 44.01;
	...
	TFile* inputFile = new TFile(Form("OutputFiles/FirstBunchRun%i.root",RunNumber), "READ");
	...
	avgEfficiencyOutputFile.open(Form("OutputFiles/FirstBunchAvgEfficiency_Run%i.dat",RunNumber));
	...
	for (const auto & arm : arms){
		for (const auto & station : stations){
			...
			SavePlotWithName(h2InterPotEfficiency,Form("OutputFiles/PlotsRun%i/FirstBunchRun%i_InterPotEfficiency_arm%i_st%i.png",RunNumber,RunNumber,arm,station));
			...
			SavePlotWithName(h2TrackEfficiencyMap,Form("OutputFiles/PlotsRun%i/FirstBunchRun%i_TrackEfficiency_arm%i_st%i.png",RunNumber,RunNumber,arm,station));
			if(noVideoOutput) gROOT->SetBatch(kTRUE);
		...
		h2Y0Correlation->DrawCopy("colz");
		cCorrelationToSave->SaveAs(Form("OutputFiles/PlotsRun%i/FirstBunchRun%i_Correlation_arm%i.png",RunNumber,RunNumber,arm));
	...
	TFile* areaFile;
	if(additionalPlots) areaFile = new TFile(Form("OutputFiles/FirstBunchAvgEfficiencyAreaRun%i.root",RunNumber), "RECREATE");
	...
	for(auto & arm : arms){
		for(auto & station : stations){
			...
			double xMax = findXMax(amplitudeFitFunction,xPeak,fitRanges[std::pair<int,int>(arm,station)].at(1),dist_fraction,0.001);
			...
			h1MeanHist->Fit("meanFitFunction","WQ","",fitRanges[std::pair<int,int>(arm,station)].at(2),2*xMax-1*fitRanges[std::pair<int,int>(arm,station)].at(2));
				// fitRanges[std::pair<int,int>(arm,station)].at(3));
			h1SigmaHist->Fit("sigmaFitFunction","WQ","",fitRanges[std::pair<int,int>(arm,station)].at(4),2*xMax-1*fitRanges[std::pair<int,int>(arm,station)].at(4));
	...
	cout << "Average efficiency data saved in " << Form("OutputFiles/FirstBunchAvgEfficiency_Run%i.dat",RunNumber) << endl;
	inputFile->Close();
	fitFile->Close();
	if(additionalPlots){
		areaFile->Close();
