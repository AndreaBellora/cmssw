...
//
/**\class EfficiencyTool_2017 EfficiencyTool_2017.cc
 RecoCTPPS/RPixEfficiencyTools/plugins/EfficiencyTool_2017.cc
...
void EfficiencyTool_2017::analyze(const edm::Event &iEvent,
  ...
  for (const auto &rpPixeltrack : *pixelLocalTracks) {
    ...
    for (const auto &pixeltrack : rpPixeltrack) {
      // if ((pixeltrack.getNDF() +4 )/2 < minNumberOfPlanesForTrack_ ||
      // (pixeltrack.getNDF() +4 )/2 > maxNumberOfPlanesForTrack_ ) continue;
      ...
      if (station == 0) {
        pixelX0_rotated =
            pixeltrack.getX0() * TMath::Cos((-8. / 180.) * TMath::Pi()) -
            pixeltrack.getY0() * TMath::Sin((-8. / 180.) * TMath::Pi());
        pixelY0_rotated =
            pixeltrack.getX0() * TMath::Sin((-8. / 180.) * TMath::Pi()) +
            pixeltrack.getY0() * TMath::Cos((-8. / 180.) * TMath::Pi());
      }
      ...
      for (const auto pln : listOfPlanes_) {
        ...
        float hitX0 = hitOnPlane[0].getGlobalCoordinates().x() +
                      hitOnPlane[0].getXResidual();
        ;
        float hitY0 = hitOnPlane[0].getGlobalCoordinates().y() +
                      hitOnPlane[0].getYResidual();
        ;
...
void EfficiencyTool_2017::fillDescriptions(
...
void EfficiencyTool_2017::getPlaneCombinations(
    const std::vector<uint32_t> &inputPlaneList, uint32_t numberToExtract,
    std::vector<std::pair<std::vector<uint32_t>, std::vector<uint32_t>>>
        &planesExtractedAndNot) {
...
float EfficiencyTool_2017::probabilityNplanesBlind(
...
float EfficiencyTool_2017::efficiencyPartialDerivativewrtPlane(
    uint32_t plane, const std::vector<uint32_t> &inputPlaneList,
    int numberToExtract, const std::map<unsigned, float> &planeEfficiency) {
...
float EfficiencyTool_2017::errorCalculation(
...
float EfficiencyTool_2017::probabilityCalculation(
    const std::map<uint32_t, float> &planeEfficiency) {
...
bool EfficiencyTool_2017::Cut(CTPPSPixelLocalTrack track, int arm,
                              int station) {
