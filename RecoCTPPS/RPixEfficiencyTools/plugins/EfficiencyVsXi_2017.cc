...
//
/**\class EfficiencyVsXi_2017 EfficiencyVsXi_2017.cc
 RecoCTPPS/RPixEfficiencyTools/plugins/EfficiencyVsXi_2017.cc
...
EfficiencyVsXi_2017::~EfficiencyVsXi_2017() {
...
void EfficiencyVsXi_2017::analyze(const edm::Event &iEvent,
  ...
  for (auto &proton : *protons) {
    ...
    for (auto &track_ptr : proton.contributingLocalTracks()) {
      ...
      try {
        ...
        if (debug_)
          std::cout << "Not the proton track!" << std::endl;
        continue;
      }
...
bool EfficiencyVsXi_2017::Cut(CTPPSLocalTrackLite track) {
  CTPPSDetId detId = CTPPSDetId(track.getRPId());
...
DEFINE_FWK_MODULE(EfficiencyVsXi_2017);